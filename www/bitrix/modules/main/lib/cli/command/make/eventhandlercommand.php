...
use Bitrix\Main\Cli\Command\Make\Service\EventHandlerService;
 ...
 */
final class EventHandlerCommand extends Command
{
	...
	protected function configure(): void
	{
		$this
			...
			->addOption('event-module', null, InputOption::VALUE_REQUIRED, 'Module id for handled event')
			->addOption('psr4', null, InputOption::VALUE_NEGATABLE, default: true)
			->setDescription('Make event handler class')
			->setHelp(<<<DESCRIPTION
			...
			<question>php bitrix.php make:eventhandler updateRating</>

			Example of creation with modules options:
			<question>php bitrix.php make:eventhandler updateRating --handler-module=my.module --event-module=another.module</>

			If you want to disable the interactive mode, then the `-n` option is used, then no questions will be asked:
			<question>php bitrix.php make:eventhandler updateRating --handler-module=my.module --event-module=another.module -n</>
			...
			<question>php bitrix.php make:eventhandler updateRating -P V2</>

			If for some reason you do not want to generate the namespace and path in the PSR-4 format, you can send the `--no-psr` option.
			<question>php bitrix.php make:eventhandler updateRating --no-psr</>
			DESCRIPTION)
		;
	...
	protected function execute(InputInterface $input, OutputInterface $output): int
	{
		...
		$eventModuleId = $this->askOrGetOption('event-module', $input, $output);
		...
		$service = new EventHandlerService($pathGenerator);
