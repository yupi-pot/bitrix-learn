...
use Bitrix\Main\Cli\Command\Make\Service\AgentService;
...
final class AgentCommand extends Command
{
	...
	protected function configure(): void
	{
		$this
			...
			<question>php bitrix.php make:agent updateRating -m my.module -n -P V2</>

			If for some reason you do not want to generate the namespace and path in the PSR-4 format, you can send the `--no-psr` option.
			<question>php bitrix.php make:agent updateRating --no-psr</>
			DESCRIPTION)
		;
	...
	protected function execute(InputInterface $input, OutputInterface $output): int
	{
		...
		$service = new AgentService($pathGenerator);
