...
class UserIndexTable extends DataManager
{
	/**
	...
	public static function merge(array $data)
	{
		...
		if (isset($updateData['SEARCH_USER_CONTENT']))
		{
			...
			$updateData['SEARCH_USER_CONTENT'] = new SqlExpression("case when " . $helper->getSha1Function(static::getTableName().".SEARCH_USER_CONTENT") . " = '{$encryptedValue}' then ".static::getTableName().".SEARCH_USER_CONTENT else '{$value}' end");
		}
		...
		if (isset($updateData['SEARCH_DEPARTMENT_CONTENT']))
		{
			...
			$updateData['SEARCH_DEPARTMENT_CONTENT'] = new SqlExpression("case when " . $helper->getSha1Function(static::getTableName().".SEARCH_DEPARTMENT_CONTENT") ." = '{$encryptedValue}' then ".static::getTableName().".SEARCH_DEPARTMENT_CONTENT else '{$value}' end");
		}
		...
		if (isset($updateData['SEARCH_ADMIN_CONTENT']))
		{
			...
			$updateData['SEARCH_ADMIN_CONTENT'] = new SqlExpression("case when " . $helper->getSha1Function(static::getTableName().".SEARCH_ADMIN_CONTENT") . " = '{$encryptedValue}' then ".static::getTableName().".SEARCH_ADMIN_CONTENT else '{$value}' end");
		}
