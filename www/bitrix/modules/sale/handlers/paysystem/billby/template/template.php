...
<?if ($params['BILLBY_HEADER']):?>
	...
	<tr>
		...
		<td style="font-size: 1.6em; font-weight: bold; text-align: center">
			<nobr><?= htmlspecialcharsbx($params['BILLBY_HEADER']) ;?> <?=Loc::getMessage('SALE_HPS_BILLBY_SELLER_TITLE', array('#PAYMENT_NUM#' => htmlspecialcharsbx($params["ACCOUNT_NUMBER"]), '#PAYMENT_DATE#' => htmlspecialcharsbx($dateValue)));?>
			</nobr>
		</td>
		<td></td>
	</tr>
...
<? if ($params["PAYMENT_DATE_PAY_BEFORE"]) { ?>
	<tr>
		<td></td>
		<td>
			<?=Loc::getMessage('SALE_HPS_BILLBY_SELLER_DATE_END', array('#PAYMENT_DATE_END#' => ConvertDateTime($params["PAYMENT_DATE_PAY_BEFORE"], FORMAT_DATE) ?: htmlspecialcharsbx($params["PAYMENT_DATE_PAY_BEFORE"])));?>
		</td>
		<td></td>
	</tr>
<? } ?>
...
if ($params['BILLBY_PAYER_SHOW'] == 'Y')
{
	...
	{
		$buyerInfoRows[] =
			Loc::getMessage('SALE_HPS_BILLBY_BUYER_DOGOVOR').': '.htmlspecialcharsbx($params['BUYER_PERSON_COMPANY_DOGOVOR']).'<br>';
	}
...
foreach ($params['BASKET_ITEMS'] as $basketItem)
{
	...
	foreach ($arCols as $columnId => $col)
	{
		...
		switch ($columnId)
		{
			...
			case 'MEASURE':
				...
				break;
			case 'PRICE':
				$data = PriceMaths::roundByFormatCurrency($basketItem['PRICE'], $basketItem['CURRENCY']);
				break;
			case 'SUM':
				$data = PriceMaths::roundByFormatCurrency($basketItemSum, $basketItem['CURRENCY']);
				break;
			case 'VAT_RATE':
				if ($basketItem['VAT_RATE'] == 0.0)
					$data = htmlspecialcharsbx(Loc::getMessage('SALE_HPS_BILLBY_TOTAL_VAT_RATE_NO'));
				else
					$data = round($basketItem['VAT_RATE'] * 100, 2)."%";
				break;
			case 'VAT_SUM':
				if ($basketItem['VAT_RATE'] == 0.0)
	...
	$sum += PriceMaths::roundByFormatCurrency($basketItem['PRICE'] * $basketItem['QUANTITY'], $currency);
	$vat = max($vat, $basketItem['VAT_RATE']);
	$totalSum += PriceMaths::roundByFormatCurrency($basketItemSum, $currency);
	$totalVatSum += PriceMaths::roundByFormatCurrency($basketItemVatSum, $currency);
	$totalSumWithVat += PriceMaths::roundByFormatCurrency($basketItemTotal, $currency);
}
...
if ($params['DELIVERY_PRICE'] > 0)
{
	...
	foreach ($arCols as $columnId => $caption)
	{
		...
		switch ($columnId)
		{
			...
			case 'MEASURE':
				...
				break;
			case 'PRICE':
				$data = PriceMaths::roundByFormatCurrency($params['DELIVERY_PRICE'], $currency);
				break;
			case 'SUM':
				$data = PriceMaths::roundByFormatCurrency($basketItemSum, $currency);
				break;
			case 'VAT_RATE':
				if ($vat == 0.0)
					$data = htmlspecialcharsbx(Loc::getMessage('SALE_HPS_BILLBY_TOTAL_VAT_RATE_NO'));
				else
					$data = roundEx($vat * 100, 2)."%";
				break;
			case 'VAT_SUM':
				if ($vat == 0.0)
					$data = htmlspecialcharsbx(Loc::getMessage('SALE_HPS_BILLBY_TOTAL_VAT_SUM_NO'));
				else
					$data = PriceMaths::roundByFormatCurrency($basketItemVatSum, $currency);
				break;
			case 'TOTAL':
				$data = PriceMaths::roundByFormatCurrency($basketItemTotal, $currency);
				break;
		}
	...
	$sum += PriceMaths::roundByFormatCurrency($params['DELIVERY_PRICE'], $currency);
	$totalSum += PriceMaths::roundByFormatCurrency($basketItemSum, $currency);
	$totalVatSum += PriceMaths::roundByFormatCurrency($basketItemVatSum, $currency);
	$totalSumWithVat += PriceMaths::roundByFormatCurrency($basketItemTotal, $currency);
}
...
if ($params['BILLBY_TOTAL_SHOW'] == 'Y')
{
	...
	foreach ($arColumnKeys as $colNum => $colCode)
	{
		...
		{
			case 'SUM':
				$value = PriceMaths::roundByFormatCurrency($totalSum, $currency);
				break;
			case 'VAT_RATE':
			...
			case 'VAT_SUM':
				if ($totalVatSum == 0.0)
					...
					$value = PriceMaths::roundByFormatCurrency($totalVatSum, $currency);
				break;
			case 'TOTAL':
				$value = PriceMaths::roundByFormatCurrency($totalSumWithVat, $currency);
				break;
			default:
				$skip = true;
		}
			...
			if (isset($tax['CODE']) && $tax['CODE'] !== 'VAT')
			{
				...
				$cells[$n][$arColumnKeys[$columnCount-2]] = htmlspecialcharsbx(sprintf(
					...
					($vat <= 0 && $tax["IS_PERCENT"] == "Y")
						? sprintf(' (%s%%)', PriceMaths::roundByFormatCurrency($tax["VALUE"], $currency))
						: ""
				));
...
<? if ($params['BILLBY_TOTAL_SHOW'] == 'Y'): ?>
	...
	if ($inWords)
		echo htmlspecialcharsbx(Number2Word_Rus(PriceMaths::roundByFormatCurrency($totalVatSum, $currency), "Y", $params['CURRENCY']));
	else
		echo htmlspecialcharsbx(SaleFormatCurrency($totalVatSum, $currency));
	unset($totalVatSum);
	?><br><br><?
...
<?if ($params['BILLBY_SIGN_SHOW'] == 'Y'):?>
	...
	<div style="position: relative">
		<table class="sign">
			...
			<tr>
				<td style="width: 150pt; font-weight: bold;"><?= htmlspecialcharsbx($params["SELLER_COMPANY_DIRECTOR_POSITION"]) ?></td>
			...
			<tr>
				<td style="width: 150pt; font-weight: bold;"><?= htmlspecialcharsbx($params["SELLER_COMPANY_ACCOUNTANT_POSITION"]) ?></td>
				<td style="width: 160pt; border-bottom: 1pt solid #000000; text-align: center; ">
					<? if (!$blank) { ?>
