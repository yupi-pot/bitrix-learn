...
/**
	 ...
	 */const d={name:"Smiles",components:{LoadingTab:a,SmilesSetTab:c,EmojiTab:r,Chip:b.Chip},directives:{lazyload:m.lazyload},props:{isOnlyEmoji:{type:Boolean,default:false}},emits:["selectSmile","selectSet"],setup(){return{ChipDesign:b.ChipDesign,ChipSize:b.ChipSize}},data(){return{smiles:[],sets:[],setSelected:0,mode:"emoji",emojiIcon:"\ud83d\ude0d"}},computed:{isShowEmoji(){return this.$Bitrix.Loc.getMessage("UTF_MODE")==="Y"},isShowSmiles(){return!this.isOnlyEmoji},isLoading(){return this.isShowSmiles&&this.smiles.length<=0},isEmojiMode(){return this.mode==="emoji"},isSmileMode(){return this.mode==="smile"},isShowTabsSelector(){if(this.isShowSmiles&&this.sets.length>1){return true}return this.isShowSmiles&&this.isShowEmoji},emojiIconStyle(){const s="bx-ui-smiles-set-emoji";if(e()){return`${s}-mac`}if(i()){return`${s}-linux`}if(n()){return`${s}-win`}return s}},created(){if(this.isShowEmoji){this.mode="emoji"}else if(this.isShowSmiles){this.mode="smile"}if(this.isShowSmiles){this.smilesController=new y(this.$Bitrix.RestClient.get());void this.smilesController.loadFromCache().then((s=>{this.smiles=s.smiles;this.sets=s.sets.map(((s,o)=>{s.selected=this.setSelected===o;return s}))}));void this.smilesController.loadFromServer().then((s=>{this.smiles=s.smiles;this.sets=s.sets.map(((s,o)=>{s.selected=this.setSelected===o;return s}))}))}},methods:{selectSet(s){if(!this.isShowSmiles){return}this.mode="smile";this.$emit("selectSet",{setId:s});void this.smilesController.changeSet(s).then((o=>{this.smiles=o;this.sets.map((o=>{o.selected=o.id===s;if(o.selected){this.setSelected=s}return o}));this.$refs.elements.scrollTop=0}))},selectSmile(s){this.$emit("selectSmile",{text:` ${s.text} `})},switchToEmoji(){if(!this.isShowEmoji){return}this.mode="emoji";this.sets.forEach((s=>{s.selected=false}))}},template:`\n\t\t<div class="bx-ui-smiles-box">\n\t\t\t<div class="bx-ui-smiles-content">\n\t\t\t\t<LoadingTab v-if="isLoading" ref="elements"/>\n\t\t\t\t<EmojiTab v-else-if="isEmojiMode" @selectSmile="selectSmile" ref="elements"/>\n\t\t\t\t<SmilesSetTab v-else-if="isSmileMode" :smiles="smiles" @selectSmile="selectSmile" ref="elements"/>\n\t\t\t</div>\n\t\t\t<template v-if="isShowTabsSelector">\n\t\t\t\t<div class="bx-ui-smiles-sets">\n\t\t\t\t\t<div v-if="isShowEmoji">\n\t\t\t\t\t\t<Chip\n\t\t\t\t\t\t\t:text="$Bitrix.Loc.getMessage('UI_VUE_SMILES_SETS_EMOJI')"\n\t\t\t\t\t\t\t:title="$Bitrix.Loc.getMessage('UI_VUE_SMILES_SETS_EMOJI')"\n\t\t\t\t\t\t\t:rounded = true\n\t\t\t\t\t\t\t:size="ChipSize.Sm"\n\t\t\t\t\t\t\t:design="isEmojiMode ? ChipDesign.Filled : ChipDesign.OUTLINE_NO_ACCENT"\n\t\t\t\t\t\t\t@click="switchToEmoji"\n\t\t\t\t\t\t/>\n\t\t\t\t\t</div>\n\t\t\t\t\t<template v-if="isShowSmiles">\n\t\t\t\t\t\t<template v-for="set in sets">\n\t\t\t\t\t\t\t<Chip\n\t\t\t\t\t\t\t\t:text="$Bitrix.Loc.getMessage('UI_VUE_SMILES_SETS_SMILES')"\n\t\t\t\t\t\t\t\t:title="set.name"\n\t\t\t\t\t\t\t\t:rounded = true\n\t\t\t\t\t\t\t\t:size="ChipSize.Sm"\n\t\t\t\t\t\t\t\t:design="isSmileMode && set.selected ? ChipDesign.Filled : ChipDesign.OUTLINE_NO_ACCENT"\n\t\t\t\t\t\t\t\t@click="selectSet(set.id)"\n\t\t\t\t\t\t\t/>\n\t\t\t\t\t\t</template>\n\t\t\t\t\t</template>\n\t\t\t\t</div>\n\t\t\t</template>\n\t\t</div>\n\t`};s.Smiles=d;s.emoji=t})(this.BX.Vue3.Components=this.BX.Vue3.Components||{},BX,BX.DexieExport,BX.Vue3.Directives,BX.UI.System.Chip.Vue);
//# sourceMappingURL=smiles.bundle.map.js