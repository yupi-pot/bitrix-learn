...
CREATE TABLE b_bp_task_archive (
	ID int GENERATED BY DEFAULT AS IDENTITY NOT NULL,
	WORKFLOW_ID varchar(32) NOT NULL,
	TASKS_DATA bytea NOT NULL,
	PRIMARY KEY (ID)
);
CREATE INDEX ix_bp_task_archive_wf_id ON b_bp_task_archive (WORKFLOW_ID);
...
CREATE INDEX ix_bp_task_archive_tasks_task_archive ON b_bp_task_archive_tasks(TASK_ID, ARCHIVE_ID);
CREATE INDEX ix_bp_task_archive_tasks_archive_completed_at ON b_bp_task_archive_tasks(ARCHIVE_ID, COMPLETED_AT);
...
CREATE UNIQUE INDEX ux_bp_robot_version_index_robot_code ON b_bp_robot_version_index (ROBOT_CODE);
CREATE INDEX ix_bp_robot_version_index_date_changed ON b_bp_robot_version_index (DATE_CHANGED);
...
CREATE INDEX ix_b_bp_workflow_template_trigger_trigger_type ON b_bp_workflow_template_trigger (trigger_type);
CREATE INDEX ix_b_bp_workflow_template_trigger_module_id_entity_document_type ON b_bp_workflow_template_trigger (module_id, entity, document_type);
...
CREATE UNIQUE INDEX ix_bp_wts_template_section_path ON b_bp_workflow_template_section(TEMPLATE_ID, SECTION_ID, PATH);
...
CREATE UNIQUE INDEX ix_bp_storage_type_code ON b_bp_storage_type(CODE);
...
CREATE UNIQUE INDEX ux_bp_storage_field_storage_code ON b_bp_storage_field(STORAGE_ID, CODE);
...
CREATE INDEX ix_bp_storage_record_data_time_storage ON b_bp_storage_record_data (CREATED_TIME, STORAGE_ID);
CREATE INDEX ix_bp_storage_record_data_document_storage ON b_bp_storage_record_data (DOCUMENT_ID, STORAGE_ID);
...
CREATE UNIQUE INDEX ux_bp_wf_template_file_template_id ON b_bp_workflow_template_file (TEMPLATE_ID, FILE_ID);
...
CREATE INDEX ix_b_bp_messenger_workflow_start_message_queue_id_status_available_at ON b_bp_messenger_workflow_start_message (QUEUE_ID, STATUS, AVAILABLE_AT);
CREATE INDEX ix_b_bp_messenger_workflow_start_message_status_available_at ON b_bp_messenger_workflow_start_message (STATUS, UPDATED_AT);
